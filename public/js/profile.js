      ...
      if (p.idcreador === uid) {
        ...
        deleteBtn.addEventListener('click', async () => {
          ...
          try {
            ...
            if (p.audioURL) {
              try {
                const audioRef = storageRef(getStorage(), p.audioURL); // use storageRef, not db ref
            ...
            // Delete icon file from storage if exists
            if (p.iconURL) {
              try {
                const iconRef = storageRef(getStorage(), p.iconURL); // use storageRef, not db ref
            ...
            // Remove podcast entry from the database
            await remove(databaseRef(getDatabase(), `podcasts/${pid}`));

            // Reload user's podcasts
...
async function openFolderModal(folderId, folderName = '') {
  ...
  try {
    ...
    listEl.innerHTML = '';
    if (items.length === 0) {
      listEl.innerHTML = '<p style="grid-column:1/-1;text-align:center;color:var(--color-blanco)">No  podcasts in this.</p>';
      return;
    }
    ...
    listEl.innerHTML = '<p style="grid-column:1/-1;text-align:center;color:var(--color-blanco)">Error charging podcasts.</p>';
  }
}
